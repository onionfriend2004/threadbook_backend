...
И промтить в ИИшку чтобы везде _LF_ был в контексте.

Не пуш скотина _CRLF_. Это может не собраться в линуксе.

## Правило JS #1

### Чтобы сгенерить новые типы нужно в .env указать URL к БД и написать в консоль:

```bash
pnpm exec prisma generate
pnpm exec prisma migrate dev --name MIGRATION-NAME-HERE
```
...
_P.S_: Не забудь конфиг, вот пример:
...
Пред вами микросервис онлайн-статусов пользователей, написанных владыкой глубин GoVнокодинга\* на `TypeScript`.
...
- POST `user/status/batch` - мы чё долбаёбы по одному тягать статусы? Можем и batch сделать. Он как бы GET, но POST. Принимает список {":username"}[] и возвращает статусы этих сказочных долбаёбов тоже в списке (_identify - username_, `це хто?` ещё немного сложнее xD, но работает примерно так же)
